...
adminip=$( curl -sS https://raw.githubusercontent.com/nyoqmuda/allow/main/accessmenu | grep $MYIP )
...
setadmin() {
#isi link git
linkinstall="https://raw.githubusercontent.com/nyoqmuda/allow/main"
rm -rf /etc/adminip/
mkdir -p /etc/adminip
...
sleep 1
cek=$( curl -sS https://raw.githubusercontent.com/nyoqmuda/allow/main/ipvps.conf | awk '{print $5}' | grep $daftar )
if [[ $daftar = $cek ]]; then
...
rm -rf /root/allow
git config --global user.email "kurita2601@gmail.com"
git config --global user.name "nyoqmuda"
git clone https://github.com/nyoqmuda/allow.git
mkdir /root/allow
cd /root/allow/
rm -rf .git
git init
...
git branch -M main
git remote add origin https://github.com/nyoqmuda/allow.git
git push -f https://${tokengit}@github.com/nyoqmuda/allow.git
...
links3="sysctl -w net.ipv6.conf.all.disable_ipv6=1 && sysctl -w net.ipv6.conf.default.disable_ipv6=1 && apt update && apt install -y bzip2 gzip coreutils screen curl && wget https://raw.githubusercontent.com/nyoqmuda/lolipop/main/setup.sh && chmod +x setup.sh && sed -i -e 's/\r$//' setup.sh && screen -S setup ./setup.sh"
...
echo "  Script Ver    : kurita_MULTIPORT"
echo -e "\033[0;34m----------------------------------------\033[0m"
